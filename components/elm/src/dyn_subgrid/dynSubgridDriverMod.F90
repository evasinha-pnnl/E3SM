...
#include "shr_assert.h"
  ...
  use elm_varctl          , only : iac_present, use_gcam_landuse
...
contains
  ...
  subroutine dynSubgrid_init(bounds, glc2lnd_vars, crop_vars)
    ...
    if (iac_present .and. use_gcam_landuse) then
  ...
  subroutine dynSubgrid_driver(bounds_proc, &
    ...
    !
    ! !USES:
    use elm_varctl           , only : use_cn, create_glacier_mec_landunit, iac_present, use_gcam_landuse
